    ...
    def make_dictionary(self, flag = ''):
        ...
        if self.__extention == "ini":
            ...
            for each in data:
                ...
                data_dict = collections.OrderedDict((x, y) for x, y in data[each].items())
    ...
    def define_config_file(self, config_file = '', flag = ''):
        ...
        if config_file != '':
            ...
            for each in list_str:
                ...
                list_str[i] = collections.OrderedDict(itertools.zip_longest(*[iter(list_str[i])] * 2))
                i = i + 1
            i = 0
            for each in self.__list_dictionary:
                j = 0
                for ea in list_str:
                    for d1 in list(each.keys()):
                        for d2 in list(ea.keys()):
                            if d1 == d2:
    ...
    def convert_file_to(self, f_name = '', f_ext = '', flag = ''):
        ...
        if f_ext == "db":
            ...
            for sect in self.__list_sections:
                ...
                ttuple = tuple((x,y) for x, y in self.__list_dictionary[i].items())
            ...
            for sect in self.__list_sections:
                dict_copy[sect] = collections.OrderedDict((x, y) for x, y in self.__list_dictionary[i].items())
                records = records + len(self.__list_dictionary[i])
                i = i + 1
